...
const els = {
  ...
  confirmBackdrop: byId('confirmBackdrop'), confirmTitle: byId('confirmTitle'), confirmMessage: byId('confirmMessage'), confirmInput: byId('confirmInput'), confirmCancel: byId('confirmCancelBtn'), confirmAccept: byId('confirmAcceptBtn'),
...
const state = { csrf:'', status:null, pollTimer:0, review:null, commissionReview:null, manageDetail:null, commissions:null, icons:new Map(), chartPackages:new Map(), axisIconSet:'english', axisScale:1.25, chartQuery:'', chartCharacter:'', chartTag:'', projectAssets:null, projectAssetSelectedId:'', projectAssetImageDataUrl:'', confirmationResolve:null, confirmationMode:'confirm' };
...
function closeConfirmation(result=null) { if(els.confirmBackdrop.hidden)return; els.confirmBackdrop.hidden=true; const resolve=state.confirmationResolve; state.confirmationResolve=null; state.confirmationMode='confirm'; syncModalBody(); if(resolve)resolve(result); }
function openConfirmation(message, options={}) { if(state.confirmationResolve)closeConfirmation(null);state.confirmationMode=options.mode||'confirm';els.confirmTitle.textContent=options.title||'确认操作';els.confirmMessage.textContent=message;els.confirmInput.hidden=state.confirmationMode!=='input';els.confirmInput.value=options.value||'';els.confirmInput.placeholder=options.placeholder||'';els.confirmInput.readOnly=Boolean(options.readOnly);els.confirmCancel.hidden=Boolean(options.hideCancel);els.confirmCancel.textContent=options.cancelText||'取消';els.confirmAccept.textContent=options.confirmText||'确认';els.confirmAccept.className=options.danger===false?'primary':'primary danger';els.confirmBackdrop.hidden=false;document.body.classList.add('modal-open');requestAnimationFrame(()=>{if(state.confirmationMode==='input'){els.confirmInput.focus();if(options.selectInput)els.confirmInput.select();}else els.confirmAccept.focus();});return new Promise((resolve)=>{state.confirmationResolve=resolve;}); }
function askConfirmation(message, options={}) { return openConfirmation(message,{...options,mode:'confirm'}); }
function askTextInput(message, options={}) { return openConfirmation(message,{...options,mode:'input'}); }
...
function riskBlock(preflight = {}) {
  ...
  const badge=document.createElement('strong'); badge.textContent=preflight.lowRisk ? '低风险' : '高风险'; wrap.appendChild(badge);
  const text=document.createElement('span'); text.textContent=preflight.lowRisk ? '格式、时段与图标转换检查通过' : (preflight.issues || []).join('；') || '等待打开后检查'; wrap.appendChild(text);
  return wrap;
}
...
async function submissionAction(id, action) { let reason='';if(action==='reject'){const input=await askTextInput('可以填写拒绝原因；不填写也可以直接拒绝。',{title:'拒绝投稿',placeholder:'拒绝原因（选填）',confirmText:'拒绝投稿'});if(input===null)return;reason=input;}await api(`/api/server/submissions/${encodeURIComponent(id)}/${action}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({reason})}); closeReview(); await loadStatus(); }
...
function renderMobileRelease(release){els.mobileReleaseVersion.value=release.version||'0.1.0';els.mobileReleaseTitle.value=release.title||'';els.mobileReleaseNotes.value=release.notes||'';els.mobileReleaseAndroidUrl.value=release.downloadLinks?.android||'';els.mobileReleaseCurrent.textContent=release.downloadLinks?.android?'已配置手机端下载链接。':'尚未填写手机端下载链接。';}
...
els.smtpResend.addEventListener('click',async()=>{if(!await askConfirmation('补发所有曾因 SMTP 未配置或发送失败而留下记录的邮件？已成功发送过的邮件不会重复发送。',{title:'补发失败邮件',confirmText:'开始补发',danger:false}))return;els.smtpResend.disabled=true;els.smtpMessage.textContent='正在逐封补发，请稍候...';try{const data=await api('/api/server/community/smtp/resend-failed',{method:'POST'});const result=data.result||{};els.smtpMessage.textContent=Number(result.attempted||0)?`补发完成：成功 ${Number(result.sent||0)} 封，仍失败 ${Number(result.failed||0)} 封。`:'没有找到需要补发的邮件。';await loadStatus({quiet:true});}catch(error){els.smtpMessage.textContent=`补发失败：${error.message}`;}finally{els.smtpResend.disabled=false;}});
...
els.copyProjectApi.addEventListener('click',async()=>{const url=new URL(els.projectApiUrl.textContent,location.origin).toString();try{await navigator.clipboard.writeText(url);els.copyProjectApi.textContent='已复制';setTimeout(()=>{els.copyProjectApi.textContent='复制地址';},1200);}catch{void askTextInput('浏览器未允许自动写入剪贴板，请在下方选择并复制地址。',{title:'复制 API 地址',value:url,readOnly:true,selectInput:true,hideCancel:true,confirmText:'关闭',danger:false});}});
